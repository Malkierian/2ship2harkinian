...
std::unordered_map<int32_t, const char*> aspectRatioPresetLabels = {
    { 0, "Off" }, { 1, "Custom" }, { 2, "Original (4:3)" }, { 3, "Widescreen (16:9)" },
    { 4, "Nintendo 3DS (5:3)" }, { 5, "16:10 (8:5)" }, { 6, "Ultrawide (21:9)" }
};
...
void RegisterResolutionWidgets() {
...
#ifdef __APPLE__
    mBenMenu->AddWidget(path, ICON_FA_INFO_CIRCLE " These settings may behave incorrectly on Retina displays.", WIDGET_TEXT)
        .Options(WidgetOptions().Color(Colors::Green));
#endif

    // Resolution visualiser
    mBenMenu->AddWidget(path, "Viewport dimensions: {} x {}", WIDGET_TEXT)
    ...
    mBenMenu->AddWidget(path, "Internal resolution: {} x {}", WIDGET_TEXT)
        .PreFunc([](WidgetInfo& info) {
            info.name = fmt::format("Internal resolution: {} x {}", gfx_current_dimensions.width, gfx_current_dimensions.height);
        });
    ...
    mBenMenu->AddWidget(path, ICON_FA_EXCLAMATION_TRIANGLE " Significant frame rate (FPS) drops may be occuring.", WIDGET_TEXT)
        .PreFunc([](WidgetInfo& info) {
            info.isHidden = !(!CVarGetInteger(CVAR_LOW_RES_MODE, 0) && IsDroppingFrames());
        })
    ...
    mBenMenu->AddWidget(path, ICON_FA_QUESTION_CIRCLE " \"N64 Mode\" is overriding these settings.", WIDGET_TEXT)
        .PreFunc([](WidgetInfo& info) {
            info.isHidden = !CVarGetInteger(CVAR_LOW_RES_MODE, 0);
        })
    ...
    mBenMenu->AddWidget(path, "AspectRationCustom", WIDGET_CUSTOM)
